...
final class SaltRoadTests: XCTestCase {
    ...
    func testPublicJsonPath() {
        ...
        let resultsPtr = SaltRoadFramework.jsonpath(queryUTF8: path.raw(), jsonUTF8: json.raw())!
    ...
    func testDownload() {
        ...
        SaltRoadFramework.download(url: url.export().0, result, nil)
        
        wait(for: [expectation], timeout: 10.0)
    }
    
    func testOCR() {
        SaltRoadFramework.ocr()
    }
