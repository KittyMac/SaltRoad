...
public func download(url urlUTF8: UTF8Ptr?,
    ...
    try? SaltRoadFrameworkPamphlet.CacertPem().description.write(toFile: cacertPath,
                                                                 atomically: false,
                                                                 encoding: .utf8)
...
@_cdecl("saltroad_ocr")
public func ocr() {
    guard let image = try? SaltRoadFrameworkPamphlet.Image6PngGzip().gunzipped() else {
